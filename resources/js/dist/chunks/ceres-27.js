...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    ...
    activeGraduationIndex() {
      var prices = this.graduatedPrices.filter(price => this.variationOrderQuantity >= price.minimumOrderQuantity);

      if (!prices.length) {
        return -1;
      }

      var price = prices.reduce((prev, current) => prev.minimumOrderQuantity > current.minimumOrderQuantity ? prev : current);
